...
如果是等值查询的情况，根据最左匹配原则，引擎会先从索引树找`a=10`的节点，因为是等值，所以我们可以利用索引树的结构准确的找到`a=10`的节点；那么以此为起点，再匹配`b=20`的节点，因为在`a`字段值确定时，`b`字段是有序的，所以我们查找`b=20`的节点时仍然可以利用索引树的结构来查找，就可以准确的找到等值的数据，**同时保证每一条读到的数据都是我们的目标数据**。
