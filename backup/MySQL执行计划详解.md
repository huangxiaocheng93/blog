...
举例来说明这个长度的计算规则：
设定如下：`test_table`表有索引`idx_a_b`是`a,b`两个字段, 其中`a`字段是`varchar(50)`，`b`字段是`bigint`，都是不允许为空的字段。
## 查询1
`select * from test_table where a='test_a' and b=0`
此时很明显会命中`idx_a_b`这个索引，那么索引长度就是`a,b`两个字段的长度：50*4+2+8，varchar一个字符占4个字节，而且由于varchar是变成字段，所以需要额外2个字节记录长度信息，`bigint`占8个字节，所以key_len=210。
## 查询2
`select * from test_table where a='test_a'`
此时也会命中`idx_a_b`这个索引，但是根据最左匹配原则，只会用到字段`a`，所以索引长度也是`a`字段的长度：50*4+2=202，所以key_len就是202。
...
还是上面的查询：`select a, b from test_table`，索引是`idx_a_b(a, b)`，假设`a,b`都是`int`型。

### 等值查询
如果是等值查询的情况，根据最左匹配原则，引擎会先从索引树找`a`字段，因为是等值，所以我们可以通过索引树准确的找到对应记录所在的节点；那么以此为起点，再匹配`b`字段，因为在`a`字段值确定时，`b`字段是有序的，所以我们查找`b`字段时仍然可以通过利用索引树的结构来查找，此时我们就可以准确的找到等值的数据，**保证每一条读到的数据都是我们需要的**。

### 非等值查询
非等值查询有多重情况，例如范围查询或者模糊查询，以范围查询距离。
