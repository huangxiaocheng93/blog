Redis的内存淘汰策略分为两个方向，**不进行数据淘汰**和**进行数据淘汰**。

## 不进行数据淘汰

**noeviction**（Redis3.0之后，默认的内存淘汰策略）

很好理解，无论如何都不做数据淘汰，如果运行内存达到设置的最大内存时，写入操作直接报错，查询和删除等操作不受影响。

## 进行数据淘汰

进行数据讨论的策略又分为两大类：在**所有数据范围内进行淘汰**和**在设置了过期时间的数据中进行淘汰**。
...
![Untitled](https://github.com/user-attachments/assets/69bab1ca-e86e-40f7-ab21-e8c1e8ef91cd)
