...
### 定义网络连接方式
例如星型，总线型，环形，现代以太网几乎都是星型结构了，其他类型看看就算了。
...
在数据链路层，会把待发送的数据再包装一次，添加帧头和帧尾，变成一个**数据帧**，数据帧就是数据链路层最小的传输单位。


### MTU
在了解数据封装之前，要先看看我们为什么需要做数据封装。
...
现代以太网的形态很多的是这种**星型**结构，所有的主机都连接到交换机，通过交换机间接的连接到一起。使用的传输介质也从同轴线进化到了双绞线。
